...
test "flex_wrap_children_with_min_main_overriding_flex_basis" {
    ...
    node_1_style.min_size.width = S.Dimension{ .Points = 55.0 };

    const node_1 = try stretch.new_node(node_1_style, &[_]S.Node{});

    var node_2_style = S.Style.default();
    node_2_style.flex_basis = S.Dimension{ .Points = 50.0 };
    node_2_style.size.height = S.Dimension{ .Points = 50.0 };
    node_2_style.min_size.width = S.Dimension{ .Points = 55.0 };
    ...
    try std.testing.expect(node_2_layout.size.width == 55.0);
    try std.testing.expect(node_2_layout.size.height == 50.0);
    try std.testing.expect(node_2_layout.location.x == 0.0);
    try std.testing.expect(node_2_layout.location.y == 50.0);
}
