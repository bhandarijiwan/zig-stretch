...
test "percentage_flex_basis_main_max_width" {
    ...
    node_1_style.flex_basis = S.Dimension{ .Percent = 0.15 };
    node_1_style.max_size.width = S.Dimension{ .Percent = 0.6 };

    const node_1 = try stretch.new_node(node_1_style, &[_]S.Node{});

    var node_2_style = S.Style.default();
    node_2_style.flex_grow = 4.0;
    node_2_style.flex_basis = S.Dimension{ .Percent = 0.1 };
    node_2_style.max_size.width = S.Dimension{ .Percent = 0.2 };
    ...
    try std.testing.expect(node_1_layout.size.width == 120.0);
    try std.testing.expect(node_1_layout.size.height == 400.0);
    ...
    try std.testing.expect(node_2_layout.size.width == 40.0);
    try std.testing.expect(node_2_layout.size.height == 400.0);
    try std.testing.expect(node_2_layout.location.x == 120.0);
