// This was generated by codegen script.

const std = @import("std");
const testing = std.testing;

test "rounding_total_fractial_nested" {
    const S = @import("stretch");
    var stretch = try S.Stretch.new(std.testing.allocator);
    defer stretch.deinit();

    var node_1_1_style = S.Style.default();
    node_1_1_style.flex_grow = 1.0;
    node_1_1_style.flex_basis = S.Dimension{ .Points = 0.3 };
    node_1_1_style.size.height = S.Dimension{ .Points = 9.9 };
    node_1_1_style.position.bottom = S.Dimension{ .Points = 13.3 };

    const node_1_1 = try stretch.new_node(node_1_1_style, &[_]S.Node{});

    var node_1_2_style = S.Style.default();
    node_1_2_style.flex_grow = 4.0;
    node_1_2_style.flex_basis = S.Dimension{ .Points = 0.3 };
    node_1_2_style.size.height = S.Dimension{ .Points = 1.1 };
    node_1_2_style.position.top = S.Dimension{ .Points = 13.3 };

    const node_1_2 = try stretch.new_node(node_1_2_style, &[_]S.Node{});

    var node_1_style = S.Style.default();
    node_1_style.flex_direction = S.FlexDirection.Column;
    node_1_style.flex_grow = 0.7;
    node_1_style.flex_basis = S.Dimension{ .Points = 50.3 };
    node_1_style.size.height = S.Dimension{ .Points = 20.3 };

    const node_1 = try stretch.new_node(node_1_style, &[_]S.Node{ node_1_1, node_1_2 });

    var node_2_style = S.Style.default();
    node_2_style.flex_grow = 1.6;
    node_2_style.size.height = S.Dimension{ .Points = 10.0 };

    const node_2 = try stretch.new_node(node_2_style, &[_]S.Node{});

    var node_3_style = S.Style.default();
    node_3_style.flex_grow = 1.1;
    node_3_style.size.height = S.Dimension{ .Points = 10.7 };

    const node_3 = try stretch.new_node(node_3_style, &[_]S.Node{});

    var node_style = S.Style.default();
    node_style.flex_direction = S.FlexDirection.Column;
    node_style.size.width = S.Dimension{ .Points = 87.4 };
    node_style.size.height = S.Dimension{ .Points = 113.4 };

    const node = try stretch.new_node(node_style, &[_]S.Node{ node_1, node_2, node_3 });

    try stretch.compute_layout(node, S.UndefinedSize());

    const node_layout = try stretch.layout(node);
    try std.testing.expect(node_layout.size.width == 87.0);
    try std.testing.expect(node_layout.size.height == 113.0);
    try std.testing.expect(node_layout.location.x == 0.0);
    try std.testing.expect(node_layout.location.y == 0.0);

    const node_1_layout = try stretch.layout(node_1);
    try std.testing.expect(node_1_layout.size.width == 87.0);
    try std.testing.expect(node_1_layout.size.height == 59.0);
    try std.testing.expect(node_1_layout.location.x == 0.0);
    try std.testing.expect(node_1_layout.location.y == 0.0);

    const node_1_1_layout = try stretch.layout(node_1_1);
    try std.testing.expect(node_1_1_layout.size.width == 87.0);
    try std.testing.expect(node_1_1_layout.size.height == 12.0);
    try std.testing.expect(node_1_1_layout.location.x == 0.0);
    try std.testing.expect(node_1_1_layout.location.y == -13.0);

    const node_1_2_layout = try stretch.layout(node_1_2);
    try std.testing.expect(node_1_2_layout.size.width == 87.0);
    try std.testing.expect(node_1_2_layout.size.height == 47.0);
    try std.testing.expect(node_1_2_layout.location.x == 0.0);
    try std.testing.expect(node_1_2_layout.location.y == 25.0);

    const node_2_layout = try stretch.layout(node_2);
    try std.testing.expect(node_2_layout.size.width == 87.0);
    try std.testing.expect(node_2_layout.size.height == 30.0);
    try std.testing.expect(node_2_layout.location.x == 0.0);
    try std.testing.expect(node_2_layout.location.y == 59.0);

    const node_3_layout = try stretch.layout(node_3);
    try std.testing.expect(node_3_layout.size.width == 87.0);
    try std.testing.expect(node_3_layout.size.height == 24.0);
    try std.testing.expect(node_3_layout.location.x == 0.0);
    try std.testing.expect(node_3_layout.location.y == 89.0);
}
