...
test "absolute_layout_percentage_bottom_based_on_parent_height" {
    ...
    node_1_style.position.top = S.Dimension{ .Percent = 0.5 };
    ...
    node_2_style.position.bottom = S.Dimension{ .Percent = 0.5 };
    ...
    node_3_style.position.top = S.Dimension{ .Percent = 0.1 };
    node_3_style.position.bottom = S.Dimension{ .Percent = 0.1 };
    ...
    try std.testing.expect(node_1_layout.location.y == 100.0);
    ...
    try std.testing.expect(node_2_layout.location.y == 90.0);

    const node_3_layout = try stretch.layout(node_3);
    try std.testing.expect(node_3_layout.size.width == 10.0);
    try std.testing.expect(node_3_layout.size.height == 160.0);
